...
2. **OpenRouter — Usage Accounting** — https://openrouter.ai/docs/guides/administration/usage-accounting — Inline `usage` object on every completion: `prompt_tokens`, `completion_tokens`, `cost`, `cost_details.upstream_inference_cost`, `cached_tokens`, `reasoning_tokens`. Receipt travels with the data, not as a sidecar fetch.
...
4. **Chrome DevTools — Network panel** — https://developer.chrome.com/docs/devtools/network/reference — Canonical receipt UI: per-request row (Name / Status / Type / Size / Time / Waterfall) collapses to one line, expands to Headers tab with three sections — General (status), Request Headers (what was sent), Response Headers (what came back). The configured-vs-applied separation is the whole structure of the panel.
...
- **Implementation cost.** Low. Server already knows resolved config; add a `meta` object to the per-turn response payload (`model`, `temperature`, `max_tokens_cap`, `tokens_used`, `finish_reason`, `provider`, `overrides_applied[]`). Brutus wires a `<TurnReceipt>` component into the existing message renderer. Mirror the OpenRouter convention: ship the receipt **inline with the completion**, not as a separate `/meta` round-trip — eliminates the "is this stale?" question.
...
- **Saturn application.** Two columns mirroring DevTools' Request/Response Headers split: **Configured** (what publisher TXT records / admin Configure page declared) vs **Applied** (what the server used for this turn). Green check where they match, amber `≠` where they diverge (e.g. coerced), red `✗` where the server ignored an override entirely. Include a "copy as JSON" button (the Vercel-style raw dump belongs here, behind a button — never in the chat stream).
