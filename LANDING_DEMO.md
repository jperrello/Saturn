...
| Saturn-68j | `7222aba` (predecessor: Saturn-zd6 / `8c91f1f`) | [`cbt.4.sec.68j-per-ip-sticky.md`](demo/recordings/cbt.4.sec.68j-per-ip-sticky.md) | **P3.** Last DoS gap on `_failover_state`. zd6 capped the map globally (`MAX_STICKY=10000`) + added TTL; one hostile IP could still burn all 10 000 slots and evict legit pins. 68j adds `MAX_STICKY_PER_IP=100` via `_StickyMap._by_ip`; per-IP eviction runs first, global FIFO drops IP-bucket references on overflow. |
